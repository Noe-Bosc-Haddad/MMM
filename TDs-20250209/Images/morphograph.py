...
## side plot: 2 graphes l'un à côté de l'autre
def sideplot(graph1, graph2, size=(16,16)):
    fig=plt.figure(figsize=size) 
    ax1 = fig.add_subplot(1,2,1)
    ax1.axis('off')
    ax1.imshow(graph1.image(),cmap="gray_r")
    ax2 = fig.add_subplot(1,2,2)
    ax2.axis('off')
    ax2.imshow(graph2.image(),cmap="gray_r")
    fig.show()

## fermeture radiale
def fermeture_radiale(graph):
    return(inf(inf(fermeture(changeAdjacency(graph,"H")),
                    fermeture(changeAdjacency(graph, "V"))),
                inf(fermeture(changeAdjacency(graph, "D1")),
                    fermeture(changeAdjacency(graph, "D2")))))   

## fermeture radiale
def ouverture_radiale(graph):
    return(sup(sup(ouverture(changeAdjacency(graph,"H")),
                    ouverture(changeAdjacency(graph, "V"))),
                sup(ouverture(changeAdjacency(graph, "D1")),
                    ouverture(changeAdjacency(graph, "D2")))))